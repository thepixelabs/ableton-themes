...
verified against all of them, not taken from documentation, because Ableton publishes none.
...
**`Clip1` to `Clip16` are identical across all 18 built-in themes.** Ableton never recolours
...
theme in `pack/` is named `PXL …`, and the prefix is what keeps them together in that list.
